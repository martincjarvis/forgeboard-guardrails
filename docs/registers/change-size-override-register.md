...
| --------- | ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `rebuild` | 2464          | 2325 production, 139 configuration. Largest single file `scripts/check-minimum-release-age.mjs` at 382 (a new check). The rest is spread thin — 13 files over 60 lines. | Turning on `noImplicitAny` and `noUncheckedIndexedAccess` touches every `.mjs` in the repository at once — 585 type errors, annotations only. Splitting it per file would produce commits that do not build, because the setting is repository-wide. That same pass is what surfaced the `readPrBody` absence defect. Two new gate 6 checks land alongside, and the eslint uplift adds the code-shape rules plus the refactors that bring eight functions back under the complexity band. | `rebuild` merges. This row is spent then; it does not carry to any later branch, and a further override needs its own row against its own branch and its own counted figure. |             |
