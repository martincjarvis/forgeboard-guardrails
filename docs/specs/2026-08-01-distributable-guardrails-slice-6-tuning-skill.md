...
commonest legitimate cause is mechanical: the round did not complete — its role
status is `hung`, `failed` or `not-started`, which slice 5's manifest states, so
the skill fills both the value and the reason
...
| A round that did not complete produces `Not measured`, never `Confirmed`     | A fixture manifest carrying a role `status` of `hung`, run through step 2                                                 |
