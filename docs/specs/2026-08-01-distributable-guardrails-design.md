...
**The seventeen gaps the journeys found are all closed.** A step that acquires
`— none —` again is a new finding and belongs in this table beside its journey
row. Two have been raised since, not by a journey but by reviewing slice 6's
requirements on slice 5 against slice 5's own scope, and both remain open — G18
and G19, the last two rows of the table below.
...
| G18 | Slice 6 R10               | **The fix brief and its dispatch are captured by nobody.** Slice 6's findings cite what was dispatched; slice 5's hand-off prints the next command, but its scope leaves writing the fix brief to a model, and its layout has no fixer directory   | — none —         | Open — which slice captures the fix brief as a round artefact is a decision for a human, not an assertion one slice makes against the other                                                       |
| G19 | Slice 6 R8                | **Manifest immutability is unstated.** Slice 6's findings are ordered by a manifest a later rewrite would alter; slice 5 writes the manifest and updates it through the round, and does not state that the completed manifest is frozen afterwards | — none —         | Open — whether slice 5 states post-completion immutability, or slice 6 stops relying on it, is a decision for a human                                                                             |
