...
<!-- cspell:ignore OpenTelemetry OTel ESLint console no-console dotnet analyser analysers redaction redacted unredacted injectable CA2254 TRACE DEBUG INFO WARN ERROR FATAL CRITICAL DI Tier TS React TypeScript -->

The React/TS side of `logging-review`. Load with [checklist.md](checklist.md);
see [../SKILL.md](../SKILL.md) for procedure. Every rule below is owned by the
[logging & diagnostics standard](../../../docs/standards/logging-diagnostics.md);
the canonical enforcer IDs live in the standard's
...
- The only admitted sink is an injected logger; verify logging is
