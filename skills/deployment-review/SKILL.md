...
description: Use when auditing or setting up a repository's versioning, packaging and deployment — per-component versions from Conventional Commits, prerelease channels per branch, immutable releases, dependency-ordered deploys, and what must be true before a release proceeds.
---

# Deployment review

Audit a repository's path from commit to release. Version derivation rests on
the `commit-messages` capability in
[docs/standards.md](../../docs/standards.md) — without checked Conventional
Commits the derivation has no input.
...
- **No taint.** A component whose dependency became a prerelease is not itself
  a prerelease.
...
2. Adopt Conventional Commits and check the message format — the derivation has
   no input without it.
...
5. Add the release checks last: artefact identity, version agreement, no
   prerelease in a release, rollback.

Do not start at step 5. Each step is the input to the next, and release checks
over a versioning scheme nobody trusts fail in ways that look like tooling
faults.
...
outranks a missing smoke test, because everything downstream trusts the
version.
...
- **A deployment-dependent test does not move into the push checks** to make a
  pipeline simpler.
- **Record a deviation** in the consuming repository's own documentation, with
