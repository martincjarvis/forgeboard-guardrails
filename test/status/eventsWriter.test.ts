...
import { appendEvent, type GateRunEvent } from "../../src/status/eventsWriter.ts";
...
test("appends events without rewriting prior lines across repeated calls", () => {
  ...
  appendEvent(dir, { ...base, timestamp: "2026-07-19T00:01:00.000Z", result: "fail" });

  const content = readFileSync(join(dir, ".forgeboard", "state", "FB-0012", "events.ndjson"), "utf8");
