 ...
 */
test(
  "a warnings-as-errors build that emits a warning rejects the commit",
  { skip: skipWithoutSemgrep },
  async () => {
    const fixture = scaffoldFixtureRepo({ statusContractEnabled: false });
    execFileSync("git", ["checkout", "-b", "feature/FB-0014-x"], {
      cwd: fixture.dir,
    });
    ...
    config.components.api.build =
      "node -e \"console.error('warning: unused variable x'); process.exit(1)\"";
