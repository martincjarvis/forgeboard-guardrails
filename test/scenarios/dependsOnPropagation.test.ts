...
test(
  "a shared-lib-only change triggers web's gates via dependsOn, catching a break",
  { skip: skipWithoutSemgrep },
  async () => {
    const fixture = scaffoldFixtureRepo({ statusContractEnabled: false });
    execFileSync("git", ["checkout", "-b", "feature/FB-0004-x"], {
      cwd: fixture.dir,
    });
    ...
    writeFileSync(
      join(fixture.dir, "src", "shared", "index.js"),
      "console.log('changed shared');\n",
    );
