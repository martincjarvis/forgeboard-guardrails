...
test("deleting a component's file still runs that component's gates", { skip: skipWithoutSemgrep }, async () => {
  const fixture = scaffoldFixtureRepo({ statusContractEnabled: false });
  execFileSync("git", ["checkout", "-b", "feature/FB-0015-x"], { cwd: fixture.dir });
  ...
  execFileSync("git", ["add", ".forgeboard/guardrails.config.json"], { cwd: fixture.dir });
  ...
  assert.equal(exitCode, 1, "a delete-only change must not skip the component's build gate");
  fixture.cleanup();
});
