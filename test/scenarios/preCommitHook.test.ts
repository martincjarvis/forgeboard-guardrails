...
function initRepoWithConfig(config: unknown): string {
  ...
  copyFileSync(
    join(pkgRoot, ".secretlintrc.json"),
...
test(
  "passes and writes status/events when statusContract is enabled and all gates pass",
  ...
  async () => {
    ...
    assert.ok(
      existsSync(join(dir, ".forgeboard", "state", "FB-0001", "status.json")),
    );
    assert.ok(
      existsSync(join(dir, ".forgeboard", "state", "FB-0001", "events.ndjson")),
    );
  },
);
...
test(
  "a missing external SAST tool blocks the commit with a clean exit, not an unhandled crash",
  {
    skip:
      semgrepAvailable() &&
      "requires semgrep to be absent to exercise the missing-tool path",
  },
  async () => {
    const dir = initRepoWithConfig({
      appName: "acme",
