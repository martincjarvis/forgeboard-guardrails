...
test("rejects a staged file with a SAST finding", { skip: skipWithoutSemgrep }, async () => {
  const fixture = scaffoldFixtureRepo({ statusContractEnabled: false });
  execFileSync("git", ["checkout", "-b", "feature/FB-0012-x"], { cwd: fixture.dir });
...
test("accepts the commit once the dangerous construct is removed", { skip: skipWithoutSemgrep }, async () => {
  const fixture = scaffoldFixtureRepo({ statusContractEnabled: false });
  execFileSync("git", ["checkout", "-b", "feature/FB-0012-y"], { cwd: fixture.dir });
  writeFileSync(
    join(fixture.dir, "src", "api", "index.js"),
    "const input = process.argv[2];\nconsole.log(input);\n"
  );
