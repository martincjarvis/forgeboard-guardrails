...
test("an unstaged secret does not block a clean staged change", { skip: skipWithoutSemgrep }, async () => {
  const fixture = scaffoldFixtureRepo({ statusContractEnabled: false });
  const file = join(fixture.dir, "src", "api", "index.js");
  execFileSync("git", ["checkout", "-b", "feature/FB-0013-x"], { cwd: fixture.dir });
  ...
  assert.equal(exitCode, 0, "gates must read the index, not the working tree");
  assert.match(readFileSync(file, "utf8"), /BEGIN RSA PRIVATE KEY/, "the unstaged edit must be restored");
  fixture.cleanup();
});

test("a staged secret still blocks even when the working tree is clean", { skip: skipWithoutSemgrep }, async () => {
  const fixture = scaffoldFixtureRepo({ statusContractEnabled: false });
  const file = join(fixture.dir, "src", "api", "index.js");
  execFileSync("git", ["checkout", "-b", "feature/FB-0013-y"], { cwd: fixture.dir });
...
test("a staged deletion does not fail the gates", { skip: skipWithoutSemgrep }, async () => {
  const fixture = scaffoldFixtureRepo({ statusContractEnabled: false });
  execFileSync("git", ["checkout", "-b", "feature/FB-0013-z"], { cwd: fixture.dir });
  ...
  assert.equal(exitCode, 0, "a removed path must not be handed to a content-reading gate");
  fixture.cleanup();
});
