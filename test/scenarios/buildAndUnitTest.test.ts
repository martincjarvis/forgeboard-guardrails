...
function setComponentBuildCommand(
  dir: string,
  component: string,
  command: string,
): void {
...
test(
  "rejects when a changed component's build fails",
  { skip: skipWithoutSemgrep },
  async () => {
    const fixture = scaffoldFixtureRepo({ statusContractEnabled: false });
    execFileSync("git", ["checkout", "-b", "feature/FB-0003-x"], {
      cwd: fixture.dir,
    });
...
test(
  "passes once the planted build failure is fixed",
  { skip: skipWithoutSemgrep },
  async () => {
    const fixture = scaffoldFixtureRepo({ statusContractEnabled: false });
    execFileSync("git", ["checkout", "-b", "feature/FB-0003-x"], {
