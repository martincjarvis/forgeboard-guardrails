...
// AWS key prefix and base64 key fixture referenced in comments/data, not prose
// cspell:ignore AKIA IDAQAB

test(
  "rejects a staged file containing a planted secret",
  { skip: skipWithoutSemgrep },
  async () => {
    const fixture = scaffoldFixtureRepo({ statusContractEnabled: false });
    execFileSync("git", ["checkout", "-b", "feature/FB-0002-x"], {
      cwd: fixture.dir,
    });
...
test(
  "accepts the commit once the secret is removed",
  { skip: skipWithoutSemgrep },
  async () => {
    const fixture = scaffoldFixtureRepo({ statusContractEnabled: false });
    execFileSync("git", ["checkout", "-b", "feature/FB-0002-x"], {
